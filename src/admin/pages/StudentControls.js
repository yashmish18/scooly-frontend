...
export default function StudentControls() {
  ...
  function filteredStudents() {
    return students.filter(s =>
      ...
      (s.course && s.course.name && s.course.name.toLowerCase().includes(search.toLowerCase())) ||
      (getName(s.batch).toLowerCase().includes(search.toLowerCase())) ||
      (getName(s.status).toLowerCase().includes(search.toLowerCase()))
    );
  }
    ...
    <div className="min-h-screen bg-pastelYellow/40 py-10">
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
          <h1 className="text-3xl font-bold text-accent">Student List</h1>
          <input
            type="text"
          ...
          <tbody>
                  {filteredStudents().map((s, i) => (
                    ...
                    >
                      ...
                      <td className="py-3 px-4">{s.course?.name || '-'}</td>
                      <td className="py-3 px-4">{getName(s.batch)}</td>
                      <td className="py-3 px-4">{getName(s.status)}</td>
              </tr>
            ))}
          </tbody>
        </table>
