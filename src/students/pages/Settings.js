...
export default function Settings() {
  ...
  useEffect(() => {
    async function fetchProfile() {
      try {
        setLoading(true);
        const res = await api.get('/profile/student'); // Adjust endpoint as needed
  ...
  const handleProfileSave = async () => {
    try {
      setLoading(true);
      await api.put('/profile/student', profile); // Adjust endpoint as needed
  ...
  const handlePasswordSave = async () => {
    try {
      setLoading(true);
      await api.put('/profile/student/password', password); // Adjust endpoint as needed
